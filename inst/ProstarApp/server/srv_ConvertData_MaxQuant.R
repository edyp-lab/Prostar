    ...
    tagList(
        ...
        shinyjs::hidden(
            div(
                ...
                p(
                    "Warning : Your original dataset may contain negative
                  values so that they cannot be logged. Please check back the
                  dataset or the log option in the first tab."
                )
            )
        )
    )
...
quantiDataTable <- reactive({
    ...
    if (isTRUE(input$selectIdent)) {
        df <- data.frame(
            as.data.frame(input$choose_quantitative_columns),
        ...
        df <- data.frame(
            Sample = as.data.frame(input$choose_quantitative_columns)
        )
