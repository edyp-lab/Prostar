    ...
    content = function(file) {
        withProgress(
            ...
            value = 0,
            {
                ...
                .protId <- (rv$dataset[[.data]])@experimentData@other$proteinId
                if (is.null(.protId)) {
                    dataToExport@experimentData@other$proteinId <- gsub(".", "_",
                        rv$proteinId,
                        fixed = TRUE
                    )
                } else {
...
BuildParamDataProcessingDT <- reactive({
    ...
    for (iData in 1:length(names(tmp.params))) {
        p <- tmp.params[[iData]]
        processName <- ifelse(is.null(names(tmp.params[[iData]])), "-",
            names(tmp.params[[iData]])
        )
